####################
#
# This is adopted from the Michal et al. 2010 paper to interact with the arduino
# controlling the NMR experiment
# Main modification:
#   - taking out any GUI-capabilities (because these used deprecated tools gtk, etc.)
#   - passing the serial handle explixcitly (not as global)
#
##################

from __future__ import division
from __future__ import print_function
# from matplotlib.backends.backend_gtk3 import NavigationToolbar2GTK3 as NavigationToolbar2  #xrs: don't need gi or
# gtk; take out all GUI functionalitly
...
import serial
import sys
#from gi.repository import Gtk  #xrs: don't need gi or gtk; take out all GUI functionalitly
import os
import tempfile
import numpy
import struct
...
# xrs: take out all GUI functionalitly and dialogs
''' 
        ...
        while runReading:
            ...
            #                sys.stdout.flush()
                ...
                while not doneRead:
                    ...
                    # done read.
                if num_read != num_points * 2:
                    return "Serial read timeout\n expected " + str(num_points * 2) + " bytes, found only: " + str(
                        num_read), None
...
def downloadProgram(fileName,
                    ardSer):  # xrs: added ardSer serial port handle parameter so I can call this fct from external scripts
    # global ardSer   # xrs: took this out
    INTER_DELAY = 150e-6
    if ardSer is None:
        ...
        if response == Gtk.ResponseType.YES:
            try:
                if arduinoBoard == 'atmega328':
                    ...
                    print('call is: ', call)
                    rsval = subprocess.call(call, shell=True)
                # this way actually calls scons to build the firmware and download it. os.chdir(ARDUINO_CODE)
...
def openDev(serialDev, myType):
    ...
    try:
        ...
        ardSer = serial.Serial(serialDev, 1000000)
        if (myType == 'hardware'):
            ardSer.setRTS(True)
            ardSer.setDTR(True)
...
# xrs: don't need gi or gtk; take out all GUI functionalitly
'''
